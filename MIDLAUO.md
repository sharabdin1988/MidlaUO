...
#### 5. Голосовой ввод текста (Speech-to-Text) и быстрые голосовые команды
- **Назначение**: набор текста и отправка команд в UO голосом без перекрытия экрана клавиатурой во время боя или ходьбы.
- **Поддержка Android**: системный `android.speech.SpeechRecognizer` работает на всех версиях от Android 7.0 до 17+:
  - На Google Pixel 7 и современных чипах (Google Tensor) распознавание идёт локально прямо на процессоре (on-device offline STT);
  - На старых телефонах работает через системный сервис Google с передачей компактного сжатого аудио (10–15 КБ).
  - При отсутствии распознавания на устройстве служба безопасно отключается без ошибок (`isRecognitionAvailable`).
- **Экранная кнопка `[ 🎤 ]` (`MobileVoiceButtonGump`)**:
  - Компактный размер 44×44 px, примагничивается к хотбарам спеллов и макросов (`AnchorableGump`).
  - Поддержка одиночного тапа (нажал -> сказал -> авто-отправка) и Push-to-Talk (удержание пальца).
  - Цветовая индикация состояния: серый (ожидание), красный пульсирующий (идёт запись), жёлтый (обработка).
- **Умные голосовые команды (Voice Aliases)**:
  - "банк" -> "bank"
  - "стража" / "гварды" -> "guards"
  - "купить" -> "vendor buy", "продать" -> "vendor sell"
